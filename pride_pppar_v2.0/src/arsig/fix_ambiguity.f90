...
!! output
      ...
      write (*, '(i5,1x,a4,2a3,2(f14.3,f8.3),2f10.1,1x,a5)') isd, AS%name, FCB%prn(isat), FCB%prn(jsat), &
  ...
  if (nwlfx_G .ne. 0) then
    write (*, '(a,a4,3i8,2(f6.1,a1,2x))') 'G_Wide/Narrow-lane FR(whole): ', AS%name, nwnfx_G, nwlfx_G, AS%nsd_G, &
      nwlfx_G*1.d2/AS%nsd_G, '%', nwnfx_G*1.d2/nwlfx_G, '%'
  endif
  if (nwlfx_R .ne. 0) then
    write (*, '(a,a4,3i8,2(f6.1,a1,2x))') 'R_Wide/Narrow-lane FR(whole): ', AS%name, nwnfx_R, nwlfx_R, AS%nsd_R, &
      nwlfx_R*1.d2/AS%nsd_R, '%', nwnfx_R*1.d2/nwlfx_R, '%'
  endif
  if (nwlfx_E .ne. 0) then
    write (*, '(a,a4,3i8,2(f6.1,a1,2x))') 'E_Wide/Narrow-lane FR(whole): ', AS%name, nwnfx_E, nwlfx_E, AS%nsd_E, &
      nwlfx_E*1.d2/AS%nsd_E, '%', nwnfx_E*1.d2/nwlfx_E, '%'
  endif
  if (nwlfx_C .ne. 0) then
    write (*, '(a,a4,3i8,2(f6.1,a1,2x))') 'C_Wide/Narrow-lane FR(whole): ', AS%name, nwnfx_C, nwlfx_C, AS%nsd_C, &
      nwlfx_C*1.d2/AS%nsd_C, '%', nwnfx_C*1.d2/nwlfx_C, '%'
  endif
  if (nwlfx_3 .ne. 0) then
    write (*, '(a,a4,3i8,2(f6.1,a1,2x))') '3_Wide/Narrow-lane FR(whole): ', AS%name, nwnfx_3, nwlfx_3, AS%nsd_3, &
      nwlfx_3*1.d2/AS%nsd_3, '%', nwnfx_3*1.d2/nwlfx_3, '%'
  endif
  if (nwlfx_J .ne. 0) then
    write (*, '(a,a4,3i8,2(f6.1,a1,2x))') 'J_Wide/Narrow-lane FR(whole): ', AS%name, nwnfx_J, nwlfx_J, AS%nsd_J, &
      nwlfx_J*1.d2/AS%nsd_J, '%', nwnfx_J*1.d2/nwlfx_J, '%'
  endif
!
!! sort ASD
  do k = 1, 2
