...
!
!! local
  integer*4 lfn, i, j, isit, isat, nprn, ierr
...
!! read parameters
  ...
  do i = 1, QN%ntot
    ...
    PM(i)%pcode(2) = pointer_string(FCB%nprn, FCB%prn, prn(isat))
    ...
    if(FCB%prn(PM(i)%pcode(2))(1:1).eq.'G')then
      PM(i)%xest = PM(i)%xest*FREQ1_G/VLIGHT
    elseif(FCB%prn(PM(i)%pcode(2))(1:1).eq.'E')then
      PM(i)%xest = PM(i)%xest*FREQ1_E/VLIGHT
    elseif(FCB%prn(PM(i)%pcode(2))(1:1).eq.'C')then
      PM(i)%xest = PM(i)%xest*FREQ1_C/VLIGHT
    elseif(FCB%prn(PM(i)%pcode(2))(1:1).eq.'J')then
