...
!! convert antenna offset from east-north-up to x-y-z in earth fixed system
  do j = 1, 2
    dump(1:3) = SITE%enu0(1:3) + SITE%enu_C(1:3, j)
...
!! Finally, form observed minus calculated, in meters.
    ...
    OB%omc(isat, 2) = OB%obs(isat, 2)/FREQ2_C*VLIGHT - phase2 + stec(2)
...
!! weight of observations
    ...
    OB%var(isat, 3) = (SITE%sigr/(vlight/FREQ1_C))**2
    OB%var(isat, 4) = (SITE%sigr/(vlight/FREQ1_C))**2
...
!! observation equation
    xrot = rot_f2j
    call partial_gnss_3(OB%npar, dloudx, drate, OB%pname, OB%ltog(1, isat), xrot, trpart, OB%amat(1, isat), &
                       LCF%prn(isat), jd, sod)
!
!! next satellite
  enddo
  ...
  if (ite .lt. 4) then
    if (SITE%ixyz .eq. 0) then
      ...
      do isat=1,LCF%nprn
        ...
        drecclk_tmp=drecclk_tmp+OB%omc(isat, 3)*FREQ1_C/VLIGHT
      enddo
      drecclk = drecclk_tmp
      if (k .ge. 1) drecclk = drecclk/(k*FREQ1_C)
      if (dabs(drecclk) .gt. 1.d-6 .or. (k .ge. 1 .and. SITE%rclock_3 .eq. 0.d0)) then
        SITE%rclock_3 = SITE%rclock_3 + drecclk*VLIGHT
        drecclk = SITE%rclock_3/VLIGHT
        goto 100
      endif
    endif
