...
!!
!
subroutine bdsmod(jd, sod, LCF, SITE, OB, SAT, IM)
  implicit none
...
!! observation equation
    xrot = rot_f2j
    call partial_gnss_C(OB%npar, dloudx, drate, OB%pname, OB%ltog(1, isat), xrot, trpart, OB%amat(1, isat), &
                       LCF%prn(isat), jd, sod)
!
!! next satellite
  enddo
...
!! receiver clock offset
  if (ite .lt. 4) then
    if (SITE%ixyz .eq. 0) then
      ...
      if (dabs(drecclk) .gt. 1.d-6 .or. (k .ge. 1 .and. SITE%rclock_C .eq. 0.d0)) then
