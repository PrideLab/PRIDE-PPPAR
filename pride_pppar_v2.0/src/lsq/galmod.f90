...
!!
!
subroutine galmod(jd, sod, LCF, SITE, OB, SAT, IM)
  implicit none
...
!! local
  ...
  real*8 xant_f(6, 2), xant_j(6, 2), xsat_j(6), dloudx(3), dump(3), &
    rot_f2j(3, 3), rot_rat(3, 3), xrot(3, 3),xsat_j_tmp(3,2)
  real*8 rf(3), date, stec(4)
  integer*4 iy, doy
  real*8 xsat_f(3),drecclk_tmp
  integer*4 iepo, iepo0,jd_ref
  real*8 sod_ref
  real*8 tmp(3,2),phwp(4),xscf(3),yscf(3),zscf(3)
  integer*4 ite
!
!! function called
  real*8 dot
...
!! convert antenna offset from east-north-up to x-y-z in earth fixed system
  do j = 1, 2
    dump(1:3) = SITE%enu0(1:3) + SITE%enu_E(1:3, j)
...
!! loop over all satellites
  do isat = 1, LCF%nprn
    if(LCF%prn(isat)(1:1) .ne. 'E') cycle
...
!! the satellite unit vectors (should be corrected for yaw error)
      call rot_scfix2j2000(SAT(isat)%typ, xsat_j, xsun, SAT(isat)%xscf, SAT(isat)%yscf, SAT(isat)%zscf)
      do j=1,2
        do i = 1, 3
          tmp(i,j)= (SAT(isat)%xyz(1, j)*SAT(isat)%xscf(i) + &
                     SAT(isat)%xyz(2, j)*SAT(isat)%yscf(i) + &
                     SAT(isat)%xyz(3, j)*SAT(isat)%zscf(i))*1.d-3
        enddo
      enddo
      do i = 1, 3
        xsat_j(i)=xsat_j(i)+tmp(i,1)
      enddo
      do j=1,2
        do i = 1, 3
          xsat_j_tmp(i,j)=xsat_j(i)-tmp(i,1)+tmp(i,j)
        enddo
      enddo
!
!! geometric distance
      do i = 1, 3
        r1(i) = xsat_j_tmp(i,1) - xant_j(i, 1)
      enddo
...
!! decide whether to iterate the delay calculation
    ...
    do i = 1, 3
      r2(i) = xsat_j_tmp(i,2) - xant_j(i, 2)
    enddo
...
!! magnitude of magnetic field & theta
    ...
    if(LCF%lioh)then
      ...
      call matmpy(xsat_j(1:3), rot_f2j, xsat_f, 1, 3, 3)
      ...
      call calstech(jd, sod, LCF, isat, SAT, IM, OB, stec)
    endif
!
...
!
!! delay
    delay(1) = delay(1) + trpdel + pcv(1) + reldel + dphwp/FREQ1_E
    delay(2) = delay(2) + trpdel + pcv(2) + reldel + dphwp/FREQ2_E
!
!! satellite clock correction, and save it for output clock estimates
    ...
    range1 = phase1 - dphwp/FREQ1_E*VLIGHT - pcv(1)*VLIGHT
    range2 = phase2 - dphwp/FREQ2_E*VLIGHT - pcv(2)*VLIGHT
!
!! Finally, form observed minus calculated, in meters.
    OB%omc(isat, 1) = OB%obs(isat, 1)/FREQ1_E*VLIGHT - phase1 + stec(1)
    OB%omc(isat, 2) = OB%obs(isat, 2)/FREQ2_E*VLIGHT - phase2 + stec(2)
...
!! weight of observations
    ...
    OB%var(isat, 3) = (SITE%sigr/(vlight/FREQ1_E))**2
    OB%var(isat, 4) = (SITE%sigr/(vlight/FREQ1_E))**2
...
!! observation equation
    xrot = rot_f2j
    call partial_gnss_E(OB%npar, dloudx, drate, OB%pname, OB%ltog(1, isat), xrot, trpart, OB%amat(1, isat), &
                       LCF%prn(isat), jd, sod)
!
!! next satellite
  enddo
  ...
  if (ite .lt. 4) then
    if (SITE%ixyz .eq. 0) then
      ...
      do isat=1,LCF%nprn
        ...
        drecclk_tmp=drecclk_tmp+OB%omc(isat, 3)*FREQ1_E/VLIGHT
      enddo
      drecclk = drecclk_tmp
      if (k .ge. 1) drecclk = drecclk/(k*FREQ1_E)
      if (dabs(drecclk) .gt. 1.d-6 .or. (k .ge. 1 .and. SITE%rclock_E .eq. 0.d0)) then
        SITE%rclock_E = SITE%rclock_E + drecclk*VLIGHT
        drecclk = SITE%rclock_E/VLIGHT
        goto 100
      endif
    endif
