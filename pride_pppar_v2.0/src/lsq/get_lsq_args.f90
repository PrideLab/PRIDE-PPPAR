...
!! define file names
  ...
  call file_name(.false., 'rcg', ' ', iy, imon, id, ih, LCF%flnrck_G)
  call file_name(.false., 'rcr', ' ', iy, imon, id, ih, LCF%flnrck_R)
  call file_name(.false., 'rce', ' ', iy, imon, id, ih, LCF%flnrck_E)
  call file_name(.false., 'rcc', ' ', iy, imon, id, ih, LCF%flnrck_C)
  call file_name(.false., 'rc3', ' ', iy, imon, id, ih, LCF%flnrck_3)
  call file_name(.false., 'rcj', ' ', iy, imon, id, ih, LCF%flnrck_J)
...
!! file names
    ...
    if (.not. lexist) then
      ...
      do j = 65, 90
        do k = 65, 90
          do l = 65, 90
            ...
            write (ddd, '(i3.3)') idoy
            SITE%obsfil(i - 11:) = &
            upper_string(key(1:4))//"00"//char(j)//char(k)//char(l)//"_R_"//yyyy//ddd//"0000_01D_30S_MO.rnx"
            inquire (file=SITE%obsfil, exist=lexist)
            if (lexist) then
              goto 50
            endif
      ...
      write (*, '(2a)') '###WARNING(get_lsq_args): observation not exist ', &
                        SITE%obsfil(:i - 12)//upper_string(key(1:4))//"00"//"XXX"//"_R_"//yyyy//ddd//"0000_01D_30S_MO.rnx"
      call exit(1)
    endif
...
!
!! add vmf3 grid
    if(SITE%map(1:3) .eq. 'VM3') call vmf3_grid(LCF%flnvmf, SITE)
!
