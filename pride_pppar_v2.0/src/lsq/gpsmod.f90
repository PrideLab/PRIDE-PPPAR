...
!! convert antenna offset from east-north-up to x-y-z in earth fixed system
  do j = 1, 2
    dump(1:3) = SITE%enu0(1:3) + SITE%enu_G(1:3, j)
...
!
!! delay
    delay(1) = delay(1) + trpdel + pcv(1) + reldel + dphwp/FREQ1_G
    delay(2) = delay(2) + trpdel + pcv(2) + reldel + dphwp/FREQ2_G
!
!! satellite clock correction, and save it for output clock estimates
    ...
    range1 = phase1 - dphwp/FREQ1_G*VLIGHT - pcv(1)*VLIGHT
    range2 = phase2 - dphwp/FREQ2_G*VLIGHT - pcv(2)*VLIGHT
!
!! Finally, form observed minus calculated, in meters.
    OB%omc(isat, 1) = OB%obs(isat, 1)/FREQ1_G*VLIGHT - phase1 + stec(1)
    OB%omc(isat, 2) = OB%obs(isat, 2)/FREQ2_G*VLIGHT - phase2 + stec(2)
...
!! weight of observations
    ...
    OB%var(isat, 3) = (SITE%sigr/(vlight/FREQ1_G))**2
    OB%var(isat, 4) = (SITE%sigr/(vlight/FREQ1_G))**2
...
!! observation equation
    xrot = rot_f2j
    call partial_gnss_G(OB%npar, dloudx, drate, OB%pname, OB%ltog(1, isat), xrot, trpart, OB%amat(1, isat), &
                       LCF%prn(isat), jd, sod)
!
!! next satellite
  enddo
  ...
  if (ite .lt. 4) then
    if (SITE%ixyz .eq. 0) then
      ...
      do isat=1,LCF%nprn
        ...
        drecclk_tmp=drecclk_tmp+OB%omc(isat, 3)*FREQ1_G/VLIGHT
      enddo
      drecclk = drecclk_tmp
      if (k .ge. 1) drecclk = drecclk/(k*FREQ1_G)
