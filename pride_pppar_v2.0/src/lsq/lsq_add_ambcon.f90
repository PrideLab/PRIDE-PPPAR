...
!! local
  ...
  real*8 is(2), iwl, rnl, fip, sodw, sodx, ini(4), op(4)
  character*2 ctyp
...
10 read (lfn, '(a)', end=100) line
  if (ctyp .eq. 'SD') then
    dname = ' '
    read (line, '(a4,1x,2(a3,1x),2(i4,4i3,f10.6,1x),2f13.0,f8.3)', err=200) cname, isat, jsat, &
...
!! add constraint to normal equation
  if (ctyp .eq. 'SD') then
    if(tmp_prn(1:1) .eq. 'G')then
      bc_G=c_G(1)*iwl/(FREQ1_G/VLIGHT)+c_G(2)*(rnl+fip)/(FREQ1_G/VLIGHT)-ini(1)+ini(2)
    elseif(tmp_prn(1:1) .eq. 'E')then
      bc_E=c_E(1)*iwl/(FREQ1_E/VLIGHT)+c_E(2)*(rnl+fip)/(FREQ1_E/VLIGHT)-ini(1)+ini(2)
    elseif(tmp_prn(1:1) .eq. 'C')then
      bc_C=c_C(1)*iwl/(FREQ1_C/VLIGHT)+c_C(2)*(rnl+fip)/(FREQ1_C/VLIGHT)-ini(1)+ini(2)
    elseif(tmp_prn(1:1) .eq. 'J')then
      bc_J=c_J(1)*iwl/(FREQ1_J/VLIGHT)+c_J(2)*(rnl+fip)/(FREQ1_J/VLIGHT)-ini(1)+ini(2)
    end if
  endif
  do i = 1, 2
    if (ix(i) .eq. 0) exit
    ...
    ic = NM%imtx + 1
    if(tmp_prn(1:1) .eq. 'G')then
      NM%norx(ir,ic)=NM%norx(ir,ic)+op(i)*1.d12*bc_G
    elseif(tmp_prn(1:1) .eq. 'E')then
      NM%norx(ir,ic)=NM%norx(ir,ic)+op(i)*1.d12*bc_E
    elseif(tmp_prn(1:1) .eq. 'C')then
      NM%norx(ir,ic)=NM%norx(ir,ic)+op(i)*1.d12*bc_C
    elseif(tmp_prn(1:1) .eq. 'J')then
      NM%norx(ir,ic)=NM%norx(ir,ic)+op(i)*1.d12*bc_J
    end if
  enddo
  if(tmp_prn(1:1) .eq. 'G')then
    NM%ltpl=NM%ltpl+1.d12*bc_G**2
  elseif(tmp_prn(1:1) .eq. 'E')then
    NM%ltpl=NM%ltpl+1.d12*bc_E**2
  elseif(tmp_prn(1:1) .eq. 'C')then
    NM%ltpl=NM%ltpl+1.d12*bc_C**2
  elseif(tmp_prn(1:1) .eq. 'J')then
    NM%ltpl=NM%ltpl+1.d12*bc_J**2
  end if
  NM%nobs = NM%nobs + 1
