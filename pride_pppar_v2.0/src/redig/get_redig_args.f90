...
!
!! local
  integer*4 i, nargs, isat, iy, imon, id, ih, im, is, iy_ses, imon_ses, id_ses, ih_ses, im_ses, is_ses, ierr
  character*3 prn(MAXSAT+20)
...
!! read header of residuals
  ...
  do while (index(line, 'END OF HEADER') .eq. 0)
    ...
    else if (index(line, 'CONFIG TIME BEG/LEN') .ne. 0) then
      read (line, *) iy_ses, imon_ses, id_ses, ih_ses, im_ses, sec_ses, seslen_ses
      RCF%seslen = ih_ses*3600.d0 + im_ses*60.d0 + sec_ses + seslen_ses
    endif
  enddo
