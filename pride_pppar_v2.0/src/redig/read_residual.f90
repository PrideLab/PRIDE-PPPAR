...
!! reports about residuals
  ...
  write (RCF%lfnstt, '(a4,1x,a4,1x,190a3)') 'NAME', 'SUMM', (RCF%prn(i), i=1, RCF%nprn)
  write (RCF%lfnstt, '(a4,1x,i4,1x,190i3)') RCF%snam, (nint(rmstbl(i)), i=0, RCF%nprn)
  write (RCF%lfnstt, '(a4,1x,a4,1x,190a3)') 'NAME', 'SUMM', (RCF%prn(i), i=1, RCF%nprn)
  if (index(RCF%obstyp, 'LC') .ne. 0) then
    write (RCF%lfnstt, '(a)') '-RMS OF RESIDUALS---PHASE(MM)'
  else
...
!! residual time series
  ...
  write (RCF%lfnstt, '(2x,a4,1x,190a3)') RCF%snam, (RCF%prn(i), i=1, RCF%nprn)
  do iepo = 1, nepo
    line = ' '
    write (line, '(i6)') iepo
    j = 7
