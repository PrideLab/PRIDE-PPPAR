...
subroutine rescode(iter, obs, n, rs, dts, vare, svh, nav, x, opt, v, H, var, vara, valres, azel, vsat, resp, ns, stat)
implicit none
...
integer*4, intent(out) :: vsat(n), ns, stat
...
! validate solution ---------------------------------------------------------
subroutine valsol(azel, vsat, n, opt, v, nv, nx, msg, stat)
implicit none
...
x=0.d0; valres=0; nslim=0; nt=0; nf=0
x(1:3)=sol%rr(1:3)
do i=1,MAXITR
    ! pseudorange residuals 
100 call rescode(i-1,obs,n,rs,dts,vare,svh,nav,x,opt,v,H,var,vara,valres,azel,vsat,resp,ns,nv)
    ...
    if(norm(dx,NX)<1d-4)then
        ...
        sol%ns=ns
        ! validate solution 
        call valsol(azel,vsat,n,opt,v,nv,NX,msg,stat1)
...
implicit none
type(sol_t), intent(out) :: sol
sol=sol_t(gtime_t(0,0.d0),0.d0,0.d0,0.d0,0,0,0)
end subroutine
...
integer*4, intent(out) :: stat
type(prcopt_t) opt_
real*8 rs(n,6),dts(n,2),var(n),azel_(n,2),resp(n)
integer*4 :: i,stat1,vsat(MAXOBS),svh(MAXOBS)
character(3) clkprn
vsat=0; opt_=opt
sol%stat=SOLQ_NONE
if(n<=0)then
...
!    write(FPCLK,*) sol%time, clkprn, dts(i,1), dts(i,2)
!enddo
