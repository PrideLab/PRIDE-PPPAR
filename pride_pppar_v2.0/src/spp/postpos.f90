...
!fpclk1=FPCLK
!open(unit=fpclk1,file='D:\Desktop\out.clk',status='replace',iostat=info)
...
!close(unit=fpclk1,status='keep')  ! output sat clock
...
if(sopt%issingle==1)then  ! output a single solution
    ...
    write(unit=6,fmt="(3F16.4)") avexyz(1), avexyz(2), avexyz(3)
...
! write header to output file 
if(flag/=0 .and. sopt%issingle==0)then
