...
subroutine get_control_parameter(flnrnx, flneph, flnrhd, &
                                 check_lc, turbo_edit, use_brdeph, use_rinex_flag, check_pc, &
  ...
  real*8 sstart, cutoff_elevation, interval, pclimit, lclimit, lglimit, lgrmslimit, &
    limit_man, max_mean_namb, min_percent, min_mean_nprn, x1(MAXEPO), x2(MAXEPO), x3(MAXEPO), x1_tmp,x2_tmp,x3_tmp

! local
  character*256 arg(30, 6), arg0, message, chr*1
...
! set default parameters
  ...
  enddo
  sstart = 0.d0
  session_length = 2880.d0*90.d0        ! at leat 3 days
  interval = 30.d0
    ...
    if (arg(ipar, 1) (1:5) .eq. '-rnxn') then
      ...
      if (.not. display_help) then
        if (nval(ipar) .eq. 4) then
          ...
          x3 = x3_tmp
          display_help = ioerr .ne. 0
        elseif (nval(ipar) .eq. 2) then
          mode = 'K'
          !! the -time arguments in tedit must be in front of the -xyz arguments 
          jd0 = modified_julday(tstart(3), tstart(2), tstart(1))
          sod0 = tstart(4)*3600.d0 + tstart(5)*60.d0 + sstart
          !! the -len arguments in tedit must be in front of the -xyz arguments 
          call timinc(jd0, sod0, session_length*1.d0, jd1, sod1)
          jd = jd0
          sod = sod0
          iepo = 0
          SITE%ikin = 0
          ...
          do while (timdif(jd, sod, jd1, sod1) .lt. -MAXWND)
            iepo = iepo + 1
            call read_kinpos(SITE, jd, sod, x1(iepo), x2(iepo), x3(iepo))
            !! the -int arguments in tedit must be in front of the -xyz arguments 
