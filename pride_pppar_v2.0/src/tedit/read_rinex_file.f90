...
subroutine read_rinex_file(flnrnx, tstart, sstart, session_length, interval, &
  ...
  integer*4 neph, tstart(1:*), session_length, suitepo
  real*4 v(suitepo, MAXSAT)
  real*8 sstart, x(MAXEPO), y(MAXEPO), z(MAXEPO), interval, lglimit, nwlimit, cutoff_elevation, pclimit
  character*(*) flnrnx, stanam
  type(brdeph) EPHEM(1:*)

  integer*4 nepo, sumepo, nsat, flagall(suitepo, MAXSAT), jd0, nobs(MAXSAT)
  real*8 ti(suitepo), ts(suitepo), obs(suitepo, MAXSAT, 6), dcb(MAXSAT, 2), bias(MAXSAT, 4)

! local
  integer*4 nprn, iepo, isat, j, k, iunit, ichn, ilast(MAXSAT), ieph, nused
  character*3 jj
...
! funtion called
  logical*1 istrue
  integer*4 modified_julday, set_flag, get_valid_unit, pointer_string, rdglschn
  real*8 timdif
...
!! tobs: fractional julday
  ...
  do i=1,30
    ...
    OB%glschn(i)=rdglschn(prn_str,jd0,tobs)
  enddo
