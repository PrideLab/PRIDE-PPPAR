...
main()
{
    ...
    local site="${4:0:4}"
    local site_l=$(echo ${site} | tr 'A-Z' 'a-z')
    local mode="${5:0:1}"
    local mode_u=$(echo ${mode} | tr 'a-z' 'A-Z')
    local interval="$6"
    local AR="${7:0:1}"
    ...
    do
        ...
        cd ./${ydoy[0]}/${ydoy[1]}
        if [ $? -eq 0 ]; then
            ProcessSingleDay $mjd "$ctrl_file" ${AR} ${site_l} ${mode_u} ${interval} || echo -e "$MSGWAR ${ydoy[*]} processing failed"
        else
...
CheckCmdArgs() { # purpose: chech whether command line arguments are right
    ...
    if [ $# -ne 7 ]; then
        ...
        local site="$4"
        local mode="$5"
        local mode_l=$(echo ${mode} | tr 'A-Z' 'a-z')
        local interval="$6"
        local AR="$7"
        ...
        if [ ! -e "$ctrl_file" ]; then
            ...
            return 1
        elif [ ${mode_l} != s -a ${mode_l} != k -a ${mode_l} != f ]; then
