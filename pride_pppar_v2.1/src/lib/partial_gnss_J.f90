...
!!
!!
subroutine partial_gnss_J(npar, ur2s, drate, parname, ltog, xrot, trpart, amat, iprn, jd, sod)
  implicit none
...
!! receiver clock epoch
    else if(parname(ipar)(1:8).eq.'RECCLK_G') then
      amat(ipar)=0.d0
    else if(parname(ipar)(1:8).eq.'RECCLK_R') then
      amat(ipar)=0.d0
    else if(parname(ipar)(1:8).eq.'RECCLK_E') then
      amat(ipar)=0.d0
    else if(parname(ipar)(1:8).eq.'RECCLK_C') then
      amat(ipar)=0.d0
    else if(parname(ipar)(1:8).eq.'RECCLK_3') then
      amat(ipar)=0.d0
    else if(parname(ipar)(1:8).eq.'RECCLK_J') then
