      ...
      do j = 1, HD%nobstyp
        ...
        elseif (prn_mat(iii)(1:1).eq.'R') then
          ...
          if (HD%obstyp(j) .eq. 'L1') then
            ...
            OB%obs(i0, 3) = obs(j) - bias(iii, biasP_index_R+9*2)
            prior_rp1 = .true.
            OB%typuse(i0, 3) = HD%obstyp(j)
          else if (HD%obstyp(j) .eq. 'C1' .and. dabs(obs(j)) .gt. 1.d7) then
            if (.not. prior_rp1) then
              OB%obs(i0, 3) = obs(j) - bias(iii, biasC_index_R+9*2)
              OB%typuse(i0, 3) = HD%obstyp(j)
            endif
          else if (HD%obstyp(j) .eq. 'P2' .and. dabs(obs(j)) .gt. 1.d7) then
            OB%obs(i0, 4) = obs(j) - bias(iii, biasP_index_R+9*3)
            prior_rp2 = .true.
            OB%typuse(i0, 4) = HD%obstyp(j)
          else if (HD%obstyp(j) .eq. 'C2' .and. dabs(obs(j)) .gt. 1.d7) then
            if (.not. prior_rp2) then
              OB%obs(i0, 4) = obs(j) - bias(iii, biasC_index_R+9*3)
              OB%typuse(i0, 4) = HD%obstyp(j)
            endif
          endif
        elseif (prn_mat(iii)(1:1).eq.'E') then
          if (HD%obstyp(j) .eq. 'L1') then
            ...
            OB%obs(i0, 2) = obs(j) - bias(iii, biasX_index_E+9)*FREQ2_E/VLIGHT
            OB%typuse(i0, 2) = HD%obstyp(j)
          else if (HD%obstyp(j) .eq. 'C1' .and. dabs(obs(j)) .gt. 1.d7) then
            OB%obs(i0, 3) = obs(j) - bias(iii, biasX_index_E+9*2)
            OB%typuse(i0, 3) = HD%obstyp(j)
          else if (HD%obstyp(j) .eq. 'C5' .and. dabs(obs(j)) .gt. 1.d7) then
            OB%obs(i0, 4) = obs(j) - bias(iii, biasX_index_E+9*3)
            OB%typuse(i0, 4) = HD%obstyp(j)
          endif
        elseif (prn_mat(iii)(1:1).eq.'C') then
          if (HD%obstyp(j) .eq. 'L2') then
            ...
            OB%obs(i0, 2) = obs(j) - bias(iii, biasI_index_C+9)*FREQ2_C/VLIGHT
            OB%typuse(i0, 2) = HD%obstyp(j)
          else if (HD%obstyp(j) .eq. 'C2' .and. dabs(obs(j)) .gt. 1.d7) then
            OB%obs(i0, 3) = obs(j) - bias(iii, biasI_index_C+9*2)
            OB%typuse(i0, 3) = HD%obstyp(j)
          else if (HD%obstyp(j) .eq. 'C6' .and. dabs(obs(j)) .gt. 1.d7) then
            OB%obs(i0, 4) = obs(j) - bias(iii, biasI_index_C+9*3)
            OB%typuse(i0, 4) = HD%obstyp(j)
          endif
        elseif (prn_mat(iii)(1:1).eq.'J') then
          if (HD%obstyp(j) .eq. 'L1') then
            ...
            OB%obs(i0, 2) = obs(j) - bias(iii, biasL_index_J+9)*FREQ2_J/VLIGHT
            OB%typuse(i0, 2) = HD%obstyp(j)
          else if (HD%obstyp(j) .eq. 'C1' .and. dabs(obs(j)) .gt. 1.d7) then
            OB%obs(i0, 3) = obs(j) - bias(iii, biasL_index_J+9*2)
            OB%typuse(i0, 3) = HD%obstyp(j)
          else if (HD%obstyp(j) .eq. 'C2' .and. dabs(obs(j)) .gt. 1.d7) then
            OB%obs(i0, 4) = obs(j) - bias(iii, biasL_index_J+9*3)
            OB%typuse(i0, 4) = HD%obstyp(j)
          endif
        endif
      enddo
