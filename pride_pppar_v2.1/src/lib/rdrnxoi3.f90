...
!! local
  ...
  real*8 sec, ds, dt, obs(MAXTYP), bias(MAXSAT, 44)
            ...
            if (HD%obstyp3_G(j) (1:2) .eq. 'L1') then
              ...
              if (obs_prio_index(1) .lt. prio_index) then
                ...
                if(bias(iii, prio_index) .eq. 0.d0) then
                  OB%obs(i0, 1) = obs(j) - bias(iii, biasW_index_G)*freq1_G/vlight
                else
                  OB%obs(i0, 1) = obs(j) - bias(iii, prio_index)*freq1_G/vlight
                endif
                OB%typuse(1, 1) = HD%obstyp3_G(j)
              endif
            elseif (HD%obstyp3_G(j) (1:2) .eq. 'L2') then
              prio_index = index(obs_prio_G, HD%obstyp3_G(j) (3:3))
              if (obs_prio_index(2) .lt. prio_index) then
                obs_prio_index(2) = prio_index
                if(bias(iii, prio_index+11) .eq. 0.d0) then
                  OB%obs(i0, 2) = obs(j) - bias(iii, biasW_index_G+11)*freq2_G/vlight
                else
                  OB%obs(i0, 2) = obs(j) - bias(iii, prio_index+11)*freq2_G/vlight
                endif
                OB%typuse(1, 2) = HD%obstyp3_G(j)
              endif
            elseif (HD%obstyp3_G(j) (1:2) .eq. 'C1') then
              prio_index = index(obs_prio_G, HD%obstyp3_G(j) (3:3))
              if (obs_prio_index(3) .lt. prio_index) then
                obs_prio_index(3) = prio_index
                if(bias(iii, prio_index+11*2) .eq. 0.d0) then
                  OB%obs(i0, 3) = obs(j) - bias(iii, biasW_index_G+11*2)
                else
                  OB%obs(i0, 3) = obs(j) - bias(iii, prio_index+11*2)
                endif
                OB%typuse(1, 3) = HD%obstyp3_G(j)
              endif
            elseif (HD%obstyp3_G(j) (1:2) .eq. 'C2') then
              prio_index = index(obs_prio_G, HD%obstyp3_G(j) (3:3))
              if (obs_prio_index(4) .lt. prio_index) then
                obs_prio_index(4) = prio_index
                if(bias(iii, prio_index+11*3) .eq. 0.d0) then
                  OB%obs(i0, 4) = obs(j) - bias(iii, biasW_index_G+11*3)
                else
                  OB%obs(i0, 4) = obs(j) - bias(iii, prio_index+11*3)
                endif
                OB%typuse(1, 4) = HD%obstyp3_G(j)
              endif
            endif
          ...
          elseif (prn_mat(iii)(1:1).eq.'E') then
            if (HD%obstyp3_E(j) (1:2) .eq. 'L1') then
              ...
              if (obs_prio_index(1) .lt. prio_index) then
                obs_prio_index(1) = prio_index
                if(bias(iii, prio_index) .eq. 0.d0) then
                  OB%obs(i0, 1) = obs(j) - bias(iii, biasX_index_E)*freq1_E/vlight 
                else
                  OB%obs(i0, 1) = obs(j) - bias(iii, prio_index)*freq1_E/vlight
                endif
                OB%typuse(3, 1) = HD%obstyp3_E(j)
              endif
            elseif (HD%obstyp3_E(j) (1:2) .eq. 'L5') then
              ...
              if (obs_prio_index(2) .lt. prio_index) then
                obs_prio_index(2) = prio_index
                if(bias(iii, prio_index+11) .eq. 0.d0) then
                  OB%obs(i0, 2) = obs(j) - bias(iii, biasX_index_E+11)*freq2_E/vlight
                else
                  OB%obs(i0, 2) = obs(j) - bias(iii, prio_index+11)*freq2_E/vlight 
                endif
                OB%typuse(3, 2) = HD%obstyp3_E(j)
              endif
            elseif (HD%obstyp3_E(j) (1:2) .eq. 'C1') then
              ...
              if (obs_prio_index(3) .lt. prio_index) then
                obs_prio_index(3) = prio_index
                if(bias(iii, prio_index+11*2) .eq. 0.d0) then
                  OB%obs(i0, 3) = obs(j) - bias(iii, biasX_index_E+11*2)
                else
                  OB%obs(i0, 3) = obs(j) - bias(iii, prio_index+11*2)
                endif
                OB%typuse(3, 3) = HD%obstyp3_E(j)
              endif
            elseif (HD%obstyp3_E(j) (1:2) .eq. 'C5') then
              ...
              if (obs_prio_index(4) .lt. prio_index) then
                obs_prio_index(4) = prio_index
                if(bias(iii, prio_index+11*3) .eq. 0.d0) then
                  OB%obs(i0, 4) = obs(j) - bias(iii, biasX_index_E+11*3)
                else
                  OB%obs(i0, 4) = obs(j) - bias(iii, prio_index+11*3)
                endif
                OB%typuse(3, 4) = HD%obstyp3_E(j)
              endif
            endif
          elseif (prn_mat(iii)(1:1).eq.'C') then
            if (HD%obstyp3_C(j) (1:2) .eq. 'L2') then
              ...
              if (obs_prio_index(1) .lt. prio_index) then
                obs_prio_index(1) = prio_index
                if(bias(iii, prio_index) .eq. 0.d0) then
                  OB%obs(i0, 1) = obs(j) - bias(iii, biasI_index_C)*freq1_C/vlight
                else
                  OB%obs(i0, 1) = obs(j) - bias(iii, prio_index)*freq1_C/vlight
                endif
                OB%typuse(4, 1) = HD%obstyp3_C(j)
              endif
            elseif (HD%obstyp3_C(j) (1:2) .eq. 'L6') then
              ...
              if (obs_prio_index(2) .lt. prio_index) then
                obs_prio_index(2) = prio_index
                if(bias(iii, prio_index+11) .eq. 0.d0) then
                  OB%obs(i0, 2) = obs(j) - bias(iii, biasI_index_C+11)*freq2_C/vlight
                else
                  OB%obs(i0, 2) = obs(j) - bias(iii, prio_index+11)*freq2_C/vlight
                endif
                OB%typuse(4, 2) = HD%obstyp3_C(j)
              endif
            elseif (HD%obstyp3_C(j) (1:2) .eq. 'C2') then
              ...
              if (obs_prio_index(3) .lt. prio_index) then
                obs_prio_index(3) = prio_index
                if(bias(iii, prio_index+11*2) .eq. 0.d0) then
                  OB%obs(i0, 3) = obs(j) - bias(iii, biasI_index_C+11*2)
                else
                  OB%obs(i0, 3) = obs(j) - bias(iii, prio_index+11*2)
                endif
                OB%typuse(4, 3) = HD%obstyp3_C(j)
              endif
            elseif (HD%obstyp3_C(j) (1:2) .eq. 'C6') then
              ...
              if (obs_prio_index(4) .lt. prio_index) then
                obs_prio_index(4) = prio_index
                if(bias(iii, prio_index+11*3) .eq. 0.d0) then
                  OB%obs(i0, 4) = obs(j) - bias(iii, biasI_index_C+11*3)
                else
                  OB%obs(i0, 4) = obs(j) - bias(iii, prio_index+11*3)
                endif
                OB%typuse(4, 4) = HD%obstyp3_C(j)
              endif
            endif
          elseif (prn_mat(iii)(1:1).eq.'J') then
            if (HD%obstyp3_J(j) (1:2) .eq. 'L1') then
              ...
              if (obs_prio_index(1) .lt. prio_index) then
                obs_prio_index(1) = prio_index
                if(bias(iii, prio_index) .eq. 0.d0) then
                  OB%obs(i0, 1) = obs(j) - bias(iii, biasL_index_J)*freq1_J/vlight
                else
                  OB%obs(i0, 1) = obs(j) - bias(iii, prio_index)*freq1_J/vlight
                endif
                OB%typuse(5, 1) = HD%obstyp3_J(j)
              endif
            elseif (HD%obstyp3_J(j) (1:2) .eq. 'L2') then
              ...
              if (obs_prio_index(2) .lt. prio_index) then
                obs_prio_index(2) = prio_index
                if(bias(iii, prio_index+11) .eq. 0.d0) then
                  OB%obs(i0, 2) = obs(j) - bias(iii, biasL_index_J+11)*freq2_J/vlight
                else
                  OB%obs(i0, 2) = obs(j) - bias(iii, prio_index+11)*freq2_J/vlight
                endif
                OB%typuse(5, 2) = HD%obstyp3_J(j)
              endif
            elseif (HD%obstyp3_J(j) (1:2) .eq. 'C1') then
              ...
              if (obs_prio_index(3) .lt. prio_index) then
                obs_prio_index(3) = prio_index
                if(bias(iii, prio_index+11*2) .eq. 0.d0) then
                  OB%obs(i0, 3) = obs(j) - bias(iii, biasL_index_J+11*2)
                else
                  OB%obs(i0, 3) = obs(j) - bias(iii, prio_index+11*2)
                endif
                OB%typuse(5, 3) = HD%obstyp3_J(j)
              endif
            elseif (HD%obstyp3_J(j) (1:2) .eq. 'C2') then
              ...
              if (obs_prio_index(4) .lt. prio_index) then
                obs_prio_index(4) = prio_index
                if(bias(iii, prio_index+11*3) .eq. 0.d0) then
                  OB%obs(i0, 4) = obs(j) - bias(iii, biasL_index_J+11*3)
                else
                  OB%obs(i0, 4) = obs(j) - bias(iii, prio_index+11*3)
                endif
                OB%typuse(5, 4) = HD%obstyp3_J(j)
              endif
            endif
          endif
        enddo
