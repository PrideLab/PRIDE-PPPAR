...
!! local
  ...
  real*8 sod0, sod1, is
  integer*4 iyear0, idoy0, isod0, iy0, imon0, id0, iyear1, idoy1, isod1, iy1, imon1, id1
      ...
      do while (.true.)
        ...
        if (iprn(1:1) .eq. 'G') then
          ...
          elseif(index(obs_type, 'L2') .ne. 0) then
            read (line, '(70x,f21.15)') bias(ii, index_obs+9)
            bias(ii, index_obs+9) = bias(ii, index_obs+9)*VLIGHT*1d-9
          elseif(index(obs_type, 'C1') .ne. 0) then
            read (line, '(70x,f21.15)') bias(ii, index_obs+9*2)
            bias(ii, index_obs+9*2) = bias(ii, index_obs+9*2)*VLIGHT*1d-9
          elseif(index(obs_type, 'C2') .ne. 0) then
            read (line, '(70x,f21.15)') bias(ii, index_obs+9*3)
            bias(ii, index_obs+9*3) = bias(ii, index_obs+9*3)*VLIGHT*1d-9
          endif
        elseif (iprn(1:1) .eq. 'R') then ! read only code bias
          index_obs = index(obs_prio_R,obs_type(3:3))
          if(index_obs .eq. 0) cycle
          if(index(obs_type, 'C1') .ne. 0) then
            read (line, '(70x,f21.15)') bias(ii, index_obs+9*2)
            bias(ii, index_obs+9*2) = bias(ii, index_obs+9*2)*VLIGHT*1d-9
          elseif(index(obs_type, 'C2') .ne. 0) then
            read (line, '(70x,f21.15)') bias(ii, index_obs+9*3)
