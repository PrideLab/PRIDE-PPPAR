...
!!
  ...
  real*8 dwnd, sod, sec, sodc, deltax(3), xsat(6), bias(MAXSAT, 36)
  character*1 cin
...
!! read pppar biases
    if (jd .gt. jd_sav) then
      bias = 0.d0
      call read_bias(LCF%flnfcb, bias, jd*1.d0, (jd+1)*1.d0)
      jd_sav = jd
    endif
!
...
!! save a priori zenith troposphere delay for each epoch
    ...
    if (PM(ipar)%iobs .gt. 0) then
      write (lfncid) 'pz'
      write (lfnrem) ipar, OB%zdd, OB%zwd, jd + sod/86400.d0,pdop
    endif
!
...
!! resolvable ambiguities
  ...
  do ipar=1,NM%ipm
    if(PM(ipar)%ptype.eq.'S' .and. PM(ipar)%iobs.gt.0) then
      ...
      if(k.ne.0 .and. any(bias(k,1:9).ne.0.d0) .and. any(bias(k,10:18).ne.0.d0)) then
