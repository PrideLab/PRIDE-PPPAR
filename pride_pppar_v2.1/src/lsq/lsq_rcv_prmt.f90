...
!! recover parameters and residuals
  ...
  do while (.true.)
    ...
    if (cid .eq. 'ob') then
      ...
      elev = elev/PI*180.d0
      azim = azim/PI*180.d0
      write (lfnres, '(a3,2f10.3,2d16.8,i3,f8.3,f9.3)') LCF%prn(isat), phase, range, wphs, wrng, iflg, elev, azim
      ...
      read (lfnrem) ipar, ntot, ipt(0), (ipt(k), k=1, ntot - 1), (cof(k), k=1, ntot), &
                    ...
                    PM(ipar)%xini, PM(ipar)%ptime(1:2), PM(ipar)%pdop
      dummy = cof(ipt(0))
      ...
      if (PM(ipar)%iobs .gt. 0) then
        ...
        if (PM(ipar)%pname(1:6) .eq. 'RECCLK') then
          ...
          write (SITE%ikin, '(i5,f10.2,a2,3f13.3,2f16.10,f13.3,i7,6(1x,i2.2),f7.2)') int(mw), (mw - int(mw))*864.d2, cid, &
                 ...
                 PM(ipar)%iobs_R, PM(ipar)%iobs_E, PM(ipar)%iobs_C, PM(ipar)%iobs_3, PM(ipar)%iobs_J, PM(ipar)%pdop
        endif
      endif
      backspace lfnrem
