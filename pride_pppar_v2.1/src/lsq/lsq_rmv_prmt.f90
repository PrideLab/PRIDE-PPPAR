...
!! get row and column to be removed
  ...
  enddo
  if (ipt(0) .eq. 0) then
    write (*, '(2a,i4)') '***ERROR(lsq_rmv_prmt): zero pivot ', PM(ipar)%pname, PM(ipar)%psat
    call exit(1)
  endif
!
  ...
  if (lfncid .ne. 0 .and. lfnrem .ne. 0) then
    if (PM(ipar)%ptype .eq. 'P') then
      ...
      write (lfnrem) ipar, nelem, ipx(0), (ipx(i), i=1, nelem - 1), &
        ...
        PM(ipar)%iobs_C, PM(ipar)%iobs_3, PM(ipar)%iobs_J, PM(ipar)%xini, PM(ipar)%ptime(1:2), PM(ipar)%pdop
