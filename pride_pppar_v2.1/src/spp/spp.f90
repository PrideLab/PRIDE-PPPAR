...
type(gtime_t) ts, te, tc  ! current time
real*8 :: tint, es(6),ee(6)
integer*4 :: i, n, ret
character(1024) :: infile(MAXFILE),outfile, buff(6), flntmp, flntmp2
character(1024) :: rnxobslist(MAXFILE),rnxnavlist(MAXFILE), obsdir, navdir
integer*4 argc, info, nsize, flnumlim
character(1024), pointer :: argv(:)
logical*1 :: isexceed, isnext
...
isnext=.false.
if(rnxobslist(1)=="" .or. rnxnavlist(1)=="" .or. .not.isnext)then
    ...
    do i=1,MAXFILE
        ...
        if(timediff(te,gtime_t(0,0.d0))>0.0 .and. timediff(tc,te)>=0.0) isexceed=.true.
        
        if(len_trim(rnxobslist(i))==0 .or. len_trim(rnxnavlist(i))==0 .or. isexceed) exit
        infile(1)=rnxobslist(i)
        infile(2)=rnxnavlist(i)
        ret=postpos(ts,te,tint,0.d0,prcopt,solopt,infile,n,outfile,'','')  ! 0-error, 1-right
        if(ret==0) exit
    enddo
