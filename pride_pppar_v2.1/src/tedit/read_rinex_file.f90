...
subroutine read_rinex_file(flnrnx, tstart, sstart, interval, &
  ...
  integer*4 nsat, flagall(nepo, MAXSAT), jd0, nobs(MAXSAT)
  real*8 ti(nepo), ts(nepo), obs(nepo, MAXSAT, 6), bias(MAXSAT, 4)
...
! **************************************************************************************** !
    if (HD%ver .eq. 3) then
      ...
      call rdrnxoi(iunit, jd0, tobs, dwnd, nprn, prn, HD, OB, bias, ierr)
    endif
    if (ierr .eq. 0) then
      obsval(0,:,:) = obsval(1,:,:)
      obsval(1,:,:) = obsval(2,:,:)
