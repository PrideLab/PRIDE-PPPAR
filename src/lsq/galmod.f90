...
!! receiver clock correction
  if (index(LCF%isbsys,'E') .ne. 0) then
    do i=1, MAXSYS+1
      if(SITE%rclock(i).ne.0.d0) exit
    enddo
    SITE%rclock(3) = SITE%rclock(i)
  endif
  drecclk = 0.d0
  drecclk = SITE%rclock(3)/VLIGHT
!
...
!! receiver clock offset
  if (ite .lt. 2 .and. index(LCF%isbsys,'E').eq.0) then
    k = 0
    drecclk_tmp = 0.d0
    do isat = 1, LCF%nprn
      if (LCF%prn(isat)(1:1) .ne. 'E') cycle
    ...
    if (dabs(drecclk) .gt. 1.d-6 .or. (k .ge. 1 .and. SITE%rclock(3) .eq. 0.d0)) then
      SITE%rclock(3) = SITE%rclock(3) + drecclk*VLIGHT
      drecclk = SITE%rclock(3)/VLIGHT
      goto 100
    !!  write (*, '(a,i7,f9.2,e15.4)') '***ERROR(galmod): abnormal drecclk at ', jd, sod, drecclk
    !!  call exit(1)
    end if
  end if
