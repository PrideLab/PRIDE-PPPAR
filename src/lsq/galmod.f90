...
!
!! delay
    delay(1) = delay(1) + trpdel + pcv(1)/VLIGHT + reldel + dphwp/FREQ1_E
    delay(2) = delay(2) + trpdel + pcv(2)/VLIGHT + reldel + dphwp/FREQ2_E
!
!! satellite clock correction, and save it for output clock estimates
    ...
    range1 = phase1 - dphwp/FREQ1_E*VLIGHT - pcv(1)
    range2 = phase2 - dphwp/FREQ2_E*VLIGHT - pcv(2)
!
...
!! weight of observations
    OB%var(isat, 1) = (SITE%sigp*SAT(isat)%var*(vlight/FREQ1_E))**2
    OB%var(isat, 2) = (SITE%sigp*SAT(isat)%var*(vlight/FREQ1_E))**2
...
!! observation equation
    call partial_gnss(OB%npar, dloudx, drate, OB%pname, OB%ltog(1, isat), rot_f2j, trpart, OB%amat(1, isat), 'E')
!
!! next satellite
  enddo
!
