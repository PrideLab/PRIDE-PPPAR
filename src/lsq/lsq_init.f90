...
! G
  if(index(LCF%sys,'G').ne.0)then
    ...
    OB%pname(OB%npar) = PM(ipar)%pname
    do isat=1,LCF%nprn
      if(LCF%prn(isat)(1:1).eq.'G') OB%ltog(OB%npar, isat) = ipar
    enddo
    NM%norx(ipar, ipar) = 1.d0/SITE%dclk0**2
    NM%iptp(ipar) = ipar
  endif
! R
  if(index(LCF%sys,'R').ne.0)then
    ip = ip + 1
    ipar = NM%nc + ip
    ...
    OB%pname(OB%npar) = PM(ipar)%pname
    do isat=1,LCF%nprn
      if(LCF%prn(isat)(1:1).eq.'R') OB%ltog(OB%npar, isat) = ipar
    enddo
    NM%norx(ipar, ipar) = 1.d0/SITE%dclk0**2
    NM%iptp(ipar) = ipar
  endif
! E
  if(index(LCF%sys,'E').ne.0)then
    ip = ip + 1
    ipar = NM%nc + ip
    ...
    OB%pname(OB%npar) = PM(ipar)%pname
    do isat=1,LCF%nprn
      if(LCF%prn(isat)(1:1).eq.'E') OB%ltog(OB%npar, isat) = ipar
    enddo
    NM%norx(ipar, ipar) = 1.d0/SITE%dclk0**2
    NM%iptp(ipar) = ipar
  endif
! C
  if(index(LCF%sys,'C').ne.0)then
    ip = ip + 1
    ipar = NM%nc + ip
...
! J
  if(index(LCF%sys,'J').ne.0)then
    ...
    OB%pname(OB%npar) = PM(ipar)%pname
    do isat=1,LCF%nprn
      if(LCF%prn(isat)(1:1).eq.'J') OB%ltog(OB%npar, isat) = ipar
    enddo
    NM%norx(ipar, ipar) = 1.d0/SITE%dclk0**2
    NM%iptp(ipar) = ipar
  endif
